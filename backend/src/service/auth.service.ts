...
export const authService = {
  async registerStep1(data: { name: string; numberPhone: string; password: string; address: string | string[] }) {
    ...
    await redis.set(`userData:${numberPhone}`, JSON.stringify({ name, password, address }), "EX", 600);
  ...
  async forgetPasswordStep2(otp: string) {
    ...
    const resetToken = jwt.sign({ numberPhone }, process.env.JWT_SECRET!, { expiresIn: "10m" });
  ...
  async changePassword(numberPhone: string, currentPassword: string, newPassword: string) {
    const user = await User.findOne({ numberPhone });
